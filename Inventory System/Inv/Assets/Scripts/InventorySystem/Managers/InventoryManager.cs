...
{
    [Header("Main Stats")]
    [SerializeField] private List<Slot> playerSlots = new List<Slot>();

    public List<Slot> PlayerSlots => playerSlots;
    ...
    /// </summary>
    public Dictionary<string, Item> ItemLibrary => itemDictionary;
    ...
    private void Start()
    {
        ...
        try
        {
            foreach (Item item in itemList)
            {
                ...
                itemDictionary.Add(item.ItemID.ToLower(), item);
            }
        ...
        {
            slot.ItemChanged += (Item item) => { OnInventorySlotChanged?.Invoke(); };
        }
    }
    ...
    #region Splitters Mergers
    public void MergeSlots(Slot selectedSlot, Slot targetSlot)
    {
    ...
    public void SplitOne(Slot selectedSlot, Slot targetSlot)
    {
    ...
    public void SplitHalf(Slot selectedSlot, Slot targetSlot)
    {
        if (targetSlot.CurrentItem == null)
        {
            targetSlot.SetItem(selectedSlot.CurrentItem, selectedSlot.ItemCount / 2, selectedSlot.SerializedData);
            selectedSlot.AdjustItemCount(-(selectedSlot.ItemCount / 2));
        }
        else if (targetSlot.CurrentItem == selectedSlot.CurrentItem)
        {
            int carryAmount = selectedSlot.ItemCount / 2;
            int slotMaxTakeAmount = targetSlot.CurrentItem.ItemMaxStackSize - targetSlot.ItemCount;
    ...
    public void SwapSlots(Slot selectedSlot, Slot targetSlot)
    {
